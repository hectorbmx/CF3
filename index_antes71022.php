...
Flight::register('db', 'PDO', array('mysql:host=localhost;dbname=riverac3_sistema','riverac3_admin','R1c2systems+'));
$db = Flight::db();
...
Flight::route('GET /reporte_socios', function(){
    ...
    //promedio semanal de ingresos    
    //$fechaComoEntero1 = strtotime($fecha2);
    $mes = date("m");
    // print_r($mes);
...
//para que los socios vean los entrenamientos del dia
//Flight::route('GET /vista_entrenos', function(){
 //  $sql="SELECT * FROM entrenos order by fecha desc";
...
//    $result=$ejecutar->Fetchall();
 ...
 //   Flight::render ('cabecera');
 //   Flight::render ('socios/vista_entrenos',array('entrenos'=>$result));
  //  Flight::render ('pie');
//para insertar los datos que se capturan en la vista crear nuevos usuarios
...
Flight::route('GET /entreno', function(){
    $sql="SELECT * FROM entrenos order by fecha desc";
    $ejecutar=Flight::db()->prepare($sql);
    $ejecutar->execute();
    ...
    Flight::render ('cabecera');
    Flight::render ('proyectos/entrenos',array('entrenos'=>$result));
    Flight::render ('pie');
});
Flight::route('POST /crear_entreno', function(){
    ...
    $sql="INSERT INTO entrenos(nombre,calentamiento,fuerza,descripcion,objetivo,fecha) VALUES (?,?,?,?,?,?)";
    $ingresar=Flight::db()->prepare($sql);
    ...
    $ingresar->bindParam(6,$fechaEntreno);
       $ingresar->execute();
       flight::redirect('/entreno');
